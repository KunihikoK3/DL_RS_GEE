...
tensorflow_dir = "C:/Users/latch/AppData/Local/R-MINI~1/envs/r-reticulate/python.exe"
...
```{r}
# load field data
field_data = readOGR("TTO_adm/TTO_adm1.shp")
...
#field_data = spTransform(field_data, crs("+proj=utm +datum=WGS84 +units=m +no_defs"))

crs(field_data) = crs("+proj=utm +datum=WGS84 +units=m +no_defs")
...
```{r}
# list images
img_list = list.files(path="C:/Users/latch/OneDrive/Desktop/DL_RS_GEE/Deep_learning_tutorial/2_Images", full.names=T) #removed pattern="singledate"
...
Note: a limitation to the current code we are using is that it only takes 4 bands. This is because the use of the PNG file, need to adjust the code to use tif. So we can use more bands, this is why we just get 4 bands in the [[]] here. This takes ~15min.
...
```{r}
# list samples
list_img = list.files(path=img_dir, pattern = "*.png", full.names = TRUE)
list_mask = list.files(path=class_dir, pattern = "*.png", full.names = TRUE)
length(list_img)
...
images <- tibble(
  img = list.files(path=img_dir, pattern = "*.png", full.names = TRUE),
  mask = list.files(path=class_dir, pattern = "*.png", full.names = TRUE)
) %>% 
  sample_n(2) %>% 
...
## Train
This is the final chunk run. Tensorflow was installed using 

install_tensorflow(gpu=TRUE, version=2.6) in the console. 

This gave the following warning message: 
Could not load dynamic library 'cudart64_110.dll'; dlerror: cudart64_110.dll not found

is given. [This site](https://developer.nvidia.com/cuda-downloads) provides a possible fix, but it may be that tensorflow requires a [specific GPU](https://tensorflow.rstudio.com/installation/gpu/local_gpu/#installation) to run in R.
...
fit(model,training_dataset,validation_data = validation_dataset, workers = 1, steps_per_epoch = as.integer(train_samples / batch_size), epochs = epochs,callbacks = callbacks_list, verbose=1)
